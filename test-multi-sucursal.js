...
} = require("./src/controllers/woocommerce_controller");
const { getAllSucursales } = require("./src/config/sucursales_credenciales");
...
async function testMultiSucursal() {
  ...
  try {
    ...
    // 4. Buscar archivos JSON de ejemplo en la carpeta de sucursales
    console.log("📋 Paso 4: Buscando archivos JSON de sucursales...");
    const sucursalesDir = path.join(
      __dirname,
      "examples",
    ...
    if (!fs.existsSync(sucursalesDir)) {
      console.log(`❌ No se encontró el directorio: ${sucursalesDir}`);
      return;
    }
    ...
    const jsonFiles = fs
      ...
      .slice(0, 2); // Solo probar con los primeros 2 archivos

    console.log(
      `✅ Se encontraron ${jsonFiles.length} archivos JSON para probar:`
    );
    ...
    // 5. Crear archivos JSON de prueba con estructura correcta
    console.log("📋 Paso 5: Creando archivos JSON de prueba...");
    ...
    // 6. Probar carga de productos para cada sucursal
    console.log("📋 Paso 6: Probando carga de productos por sucursal...");
    ...
    // 7. Probar obtención de productos por sucursal
    console.log("📋 Paso 7: Probando obtención de productos por sucursal...");
    ...
    console.log("📋 Paso 8: Limpiando archivos temporales...");
    try {
...
async function testProductSearch() {
  ...
  const testSkus = ["999001", "999003"]; // SKUs de nuestros productos de prueba
  const sucursalIds = [10, 11]; // Autopista y Bahía Blanca

  for (let i = 0; i < testSkus.length; i++) {
    const sku = testSkus[i];
    const sucursalId = sucursalIds[i];

    console.log(`🔎 Buscando producto SKU ${sku} en sucursal ${sucursalId}...`);
